#!/bin/bash
# Fixed script to add liquidation-heatmap location blocks to nginx config
...
if [ -z "$INSERT_LINE" ]; then
    echo "Error: Could not find 'location /' block"
    exit 1
fi
...
# Insert the config using a different method
head -n $((INSERT_LINE - 1)) "$NGINX_CONFIG" > "${NGINX_CONFIG}.new"
cat "$TEMP_CONFIG" >> "${NGINX_CONFIG}.new"
tail -n +$INSERT_LINE "$NGINX_CONFIG" >> "${NGINX_CONFIG}.new"
