#!/usr/bin/env python3
"""
Backtest Optimized High-Frequency Momentum Strategy
Uses dynamic leverage and relaxed filters for maximum volume
"""
...
def backtest_optimized(symbol, data, timeframe='5m'):
    """Backtest optimized strategy on a symbol"""
    if len(data) < 200:
        return None, f"Insufficient data: {len(data)} candles"
    
    try:
        strategy = OptimizedHighFrequencyMomentumStrategy(timeframe=timeframe)
        signals = strategy.generate_signals(data)
        
        # Count signals and leverage distribution
...
def main():
    ...
    print("="*80)
    print("OPTIMIZED HIGH-FREQUENCY MOMENTUM STRATEGY BACKTEST")
    print("="*80)
    print(f"\nStrategy: OptimizedHighFrequencyMomentumStrategy")
    print(f"Features: Dynamic Leverage (10x-20x), Relaxed Filters")
    print(f"Symbols: {len(ALL_MEMECOINS)}")
    ...
    for i, symbol in enumerate(ALL_MEMECOINS, 1):
        ...
        try:
            ...
            results, error = backtest_optimized(symbol, df, timeframe='5m')
    ...
    if results_dict:
        ...
        results_file = Path('research') / f'backtest_optimized_5m_{timestamp}.json'
        ...
        if summary_stats['successful'] > 0:
            ...
            print(f"  Total Signals Generated: {summary_stats['total_signals']}")
            print(f"  Signal-to-Trade Ratio: {summary_stats['total_signals']/summary_stats['total_trades']:.2f}" if summary_stats['total_trades'] > 0 else "  N/A")
            ...
            total_leverage_trades = sum(summary_stats['leverage_counts'].values())
            for lev, count in summary_stats['leverage_counts'].items():
                pct = (count / total_leverage_trades * 100) if total_leverage_trades > 0 else 0
                print(f"  {lev}: {count} trades ({pct:.1f}%)")
